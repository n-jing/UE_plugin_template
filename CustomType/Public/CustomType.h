...
UCLASS()
class CUSTOMTYPE_API UCustomK2Node : public UK2Node
{
	GENERATED_BODY()
	...
	virtual void AllocateDefaultPins()override
	{
		...
		UEnum* const MethodEnum = FindObjectChecked<UEnum>(ANY_PACKAGE, TEXT("ETypeName"), true);
		UEdGraphPin* const EnumPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Byte, MethodEnum, TEXT("TypeName"));
		if (CurrentName.IsEmpty())
		{
			EnumPin->DefaultValue = MethodEnum->GetNameStringByValue(static_cast<int>(ETypeName::None));
